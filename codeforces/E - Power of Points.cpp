...
#define visar(x) for(auto i:x) cout<<i<<" "; cout<<endl;
#define pb push_back
...
int32_t main()
{
    ...
    for(int i=1; i<=n; i++){
        int ans=0;
        ans = n+v[i-1].first*(2*(i)-n)-prex[i]+prex[n]-prex[i];
        ar[v[i-1].second]=ans;
       // cout<<v[i-1]<<" "<<prex[i]<<" "<<prex[n]-prex[i]<<endl;
